...
beforeAll(async () => {
    await new Promise((resolve) => {
        xNode = new Node.default('127.0.0.1', 50002, resolve);
    })
    
    await new Promise((resolve) => {
        yNode = new Node.default('127.0.0.1', 50003, resolve);
    })
});
...
describe('Sending Pings', () => {
    ...
    test('sends ping message and gets positive response', async () => {
        ...
        await xNode.ping(yNode.encapsulateSelf())
            .then((res) => expect(res).toEqual({ result: true, ping: true }))
            .catch((error) => expect(error).toBeUndefined());
        
        expect(messageSpy).toHaveBeenNthCalledWith(1, expect.any(Buffer), { "address": "127.0.0.1", "family": "IPv4", "port": 50002, "size": 164 });
        expect(messageSpy).toHaveBeenNthCalledWith(2, expect.any(Buffer), { "address": "127.0.0.1", "family": "IPv4", "port": 50003, "size": 203 });
